...
class TestProjectSwitchEditPersistence(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_queue_switch_persists_edit_still_pending_in_editor_debounce(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            window.sub_editor.txt_content.setPlainText("E1_SWITCH_PERSIST_TEST")
            self.assertTrue(window.sub_editor.current_editor._debounce.isActive())
            self.assertEqual(
                window.sub_editor.all_segments[0]["text"], "original A"
            )
            ...
            self.assertIn("open:project-b.ai-subtitle", events)
            self.assertIn("save:Project A", events)
            self.assertLess(
                events.index("save:Project A"),
                events.index("open:project-b.ai-subtitle"),
            )

            window.on_queue_item_clicked(str(video_a))
            self.assertEqual(
                window.sub_editor.all_segments[0]["text"], "E1_SWITCH_PERSIST_TEST"
            )
