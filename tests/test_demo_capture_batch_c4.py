...
from tools.demo_capture.cli import _run_batch
...
class TestDemoCaptureBatchC4(unittest.TestCase):
    def setUp(self):
        ...
        for scenario in self.scenarios:
            scenario.profile = MagicMock(fps=10)
    ...
    def test_tc243_all_or_nothing(self):
        ...
        # CLI batch wiring must stage all scenarios before BatchProcessor commits.
        def cli_runner(scenario, mode, staging_dir, *dependencies):
            staged_file = staging_dir / scenario.output.filename
            staged_file.write_text("dummy")
            if scenario.id == "B":
                raise RuntimeError("B Failed")
        ...
        with patch("tools.demo_capture.cli.capture_to_staging", side_effect=cli_runner):
            _run_batch(
                self.registry,
    ...
    def test_tc246_commit_phase_starts_last(self):
        ...
        # The CLI composition must preserve the same commit boundary.
        events.clear()
